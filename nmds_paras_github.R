...
dado=read.delim("nmds_paras.csv", sep= ";", header = TRUE)
names(dado)
...
dados = dado%>%
  select(tratamento, area, hy_sp2:hy_sp17)%>%
  drop_na()
summary(dados)
...
###plotting figure 4A
tiff("nmds_paras.tiff", units="in", width=10, height=8, res=300)
...
legend("topleft", "stress = 0.1077", bty = "n", cex = 1) 
